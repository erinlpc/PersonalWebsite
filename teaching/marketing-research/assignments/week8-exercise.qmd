...
1.  Load a slightly altered version of the housing dataset from Exam 1
...
1.  **Simple regression** compares a mean-only model (Model C) to a slope + intercept model (Model A)
2.  **PRE** still works the same way: $(SSE_C - SSE_A) / SSE_C$
3.  In simple regression, **PRE = R²** from the summary output
4.  **Centering X** changes the interpretation of the intercept but not the slope
5.  **Confidence intervals** for the intercept are tightest when X is centered near the data's center
